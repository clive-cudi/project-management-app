...
export const TableRow = ({
  ...
  const regexCmpntRef: RegExp = new RegExp(/__component@[a-zA-Z_-]+/);
  ...
  return (
    ...
    >
        {/* add functionality for rendering special components in cells since the arrays are limited to strings */}
      ...
      {rowData.length > 0 ? (
        ...
        if (typeof cell === "string") {
            if (regexCmpntRef.test(cell)) {
                ...
                console.log(`Rendering componentID: ${(cell.match(/[^__component@](.)*/) ?? [])[0]}`);
