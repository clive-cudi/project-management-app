...
const verify = (req, res, next) => {
  const {
    ...
    gender = "",
    skills = [],
    address: { country = "KE", location = "Nairobi", street = "" },
  } = req.body;

  // usertype = "individual" | "organization"

  User.findOne({ uid: usertoken.uid, email: usertoken.email }).then(
    async (user) => {
      if (user) {
        User.updateOne(
          { uid: usertoken.uid },
          {
            $set: {
              ...
              "info.skills": Array.from([
                ...new Set([...user.info.skills, ...skills]),
              ]),
              "info.gender": gender,
              "info.timezones": {
                default: timezone,
                other: Array.from([
                  ...new Set(
                    user.info.timezones.other.includes(timezone)
                      ? user.info.timezones.other
                      : [...user.info.timezones.other, timezone]
                  ),
                ]),
              },
              "info.phone": phone,
...
const enableTwoFactorAuthStep2 = (req, res, next) => {
  ...
  if (isVerified) {
    User.findOne({ email: usertoken.email, uid: usertoken.uid })
      .then((user) => {
        if (user) {
...
const validate2FAtoken = (req, res, next) => {
  ...
  User.findOne({ email: usertoken.email })
    .then((user) => {
      if (user) {
...
const googleAuth = async (req, res, next) => {
  ...
  User.findOne({ email: authPayload.email })
    .then((user) => {
      if (user) {
        // user exists
        console.log("User Exists");
        ...
        User.findOneAndUpdate(
          ...
          .catch((err) => {
            ...
            return res.status(200).json({
              success: false,
              message:
                "Google Auth failed. Update of google auth user credentials failed",
              usertoken: {
                user: { ...dataToInclude, twoFA: { status: twoFA.status } },
                token: token,
              },
